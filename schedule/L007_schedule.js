...
exports.scheduleInsert = () => {
    schedule.scheduleJob(process.env.L007_TIME, async function () {
        let rtnResult = {};
        ...
        try {
            const result = await db.sequelize.transaction(async (t) => {
                ...
                let rslt = await db.sequelize.query(
                    ...
                    'where dti.motie_data_request.gubun = \'LOG_E\' and dti.motie_data_request.state = \'200\';'
                    ,{
                    ...
                    if(users.length) {
                        for (let user of users) {
                            ...
                            let value = makejson.makeReqData_L007('L007', data);
                            winston.debug(JSON.stringify(value));
                            setTimeout(function() {
                                httpcall.Call('get', process.env.L007_ADDRESS, value, async function (err, res) {
                                    if (res) {
